...
pairChoose env e choice =  do (s1,t1) <- ti env e
                              l <- freshTVbl "a"
                              r <- freshTVbl "b"
                              s2 <- mgu t1 (l `TCom` r)
                              return (s2 `after` s1, apply s2 (if choice
                                                               then l
                                                               else r))
...
ti env (EFst e)       = pairChoose env e True
ti env (ESnd e)       = pairChoose env e False

ti env ENil            = do 
                           var <- freshTVbl "a"
                           return (empSubst, TList var)
ti env (e1 `ECons` e2) = do
                           (s1, t1) <- ti env e1
                           (s2, t2) <- ti (apply s1 env) e2
                           s3 <- mgu t2 (TList (apply s2 t1))
...
ti env (EDcons e)      = do
                           ...
                           var <- freshTVbl "a"
                           s2 <- mgu t1 (TList var)
                           return (s2 `after` s1, (apply s2 var) `TCom` (apply s2 (TList var)))

ti env (ERec x e1 e2)  = do
                           var <- freshTVbl "a"
                           ti (env \\ (x, Forall [] var)) (ELet x e1 e2)
