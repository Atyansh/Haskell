...
\begin{code}
data FiniteChan a = Chan (MVar ([a],(MVar Int),Int)) (MVar Int) (MVar Int)
\end{code}
...
newFiniteChan capacity = do
                           size <- newMVar 0
                           x <- newMVar ([],size,capacity)
                           canRead <- newEmptyMVar
                           if (capacity == 0)
                           then do
                                  canWrite <- newEmptyMVar
                                  return (Chan x canRead canWrite)
                           else do
                                  canWrite <- (newMVar 1)
                                  return (Chan x canRead canWrite)
\end{code}
...
readFiniteChan (Chan c canRead canWrite) = do
                                            takeMVar canRead
                                            (q,size,capacity) <- takeMVar c
                                            s <- takeMVar size
                                            case q of
                                              (h:t) -> do
                                                       putMVar size (s-1)
                                                       putMVar c (t,size,capacity)
                                                       if (s == capacity && s == 1)
                                                       then do
                                                              putMVar canWrite 1
                                                              return h
                                                       else if (s == capacity && s /= 1)
                                                       then do 
                                                              putMVar canRead 1
                                                              putMVar canWrite 1
                                                              return h
...
writeFiniteChan (Chan c canRead canWrite) x = do
                                                takeMVar canWrite
                                                (q,size,capacity) <- takeMVar c
                                                s <- takeMVar size
                                                putMVar size (s+1)
                                                if (s == 0 && s == (capacity - 1))
                                                then do
                                                       putMVar c (q++[x],size,capacity)
                                                       putMVar canRead 1
                                                else if (s /= 0 && s == (capacity - 1))
                                                then do
                                                       putMVar c (q++[x],size,capacity)
                                                else if (s == 0 && s /= capacity - 1)
                                                then do
