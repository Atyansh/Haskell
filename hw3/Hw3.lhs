...
> bstInsert k' v' (Bind k v l r) = if (k' < k)
>                                  then rotate (Bind k v (bstInsert k' v' l) r)
>                                  else (if (k' > k)
>                                        then rotate (Bind k v l (bstInsert k' v' r))
>                                        else (Bind k' v' l r))

> rotate Emp = Emp
> rotate (Bind k v l r) = if ((abs ((height l) - (height r))) <= 1)
>                         then (Bind k v l r)
>                         else (if (((height l) - (height r)) > 1)
>                               then rotateRight (Bind k v l r)
>                               else rotateLeft (Bind k v l r))

> rotateRight (Bind k v l r) = if ((balance l) < 0)
>                              then (let (Bind k' v' l' r') = rotateLeft l in
>                                    (Bind k' v' l' (Bind k v r' r)))
>                              else (let (Bind k' v' l' r') = l in
>                                    (Bind k' v' l' (Bind k v r' r)))

> rotateLeft (Bind k v l r) = if ((balance r) > 0)
>                             then (let (Bind k' v' l' r') = rotateRight r in
>                                   (Bind k' v' (Bind k v l l') r'))
>                             else (let (Bind k' v' l' r') = r in
>                                   (Bind k' v' (Bind k v l l') r'))
...
>                              then rotate (Bind k' v' (insert' (Bind k v l' r') l) r)
>                              else (if (k > k')
>                                    then rotate (Bind k' v' l (insert' (Bind k v l' r') r))
>                                    else rotate (Bind k v l' r'))

> bstDelete :: (Ord k) => k -> BST k v -> BST k v
> bstDelete k' Emp            = Emp
> bstDelete k' (Bind k v l r) = if (k' < k)
>                               then rotate (Bind k v (bstDelete k' l) r)
>                               else (if (k' > k)
>                                     then rotate (Bind k v l (bstDelete k' r))
>                                     else case (l,r) of
>                                       (Emp, Emp) -> Emp
>                                       (Emp, r') -> r'
>                                       (l', Emp) -> l'
>                                       (l', r') -> rotate (insert' l' r'))
...
> balance Emp            = 0
> balance (Bind _ _ l r) = (height l) - (height r)
...
> genBal = frequency [ (1, return Emp)
>                    , (7, liftM3 bstInsert arbitrary arbitrary genBal)]
...
>   if (xs == [] || (binary xs) == 0)
>   then ( (binary (sampleN out) == 0) && (binary (sample1 cout) == (binary (sample1 cin))))
>   else ((binary (sampleN out) == binary xs - binary (sample1 cin)) && (binary (sample1 cout) == 0))
>   where (out, cout) = bitSubtractor (cin, map lift0 xs)
...
> bitSubtractor (bin, xs) = if (binary (sampleN xs) == 0)
>                           then (xs, bin)
>                           else bSub (bin, xs)

> bSub (bin, []) = ([], bin)
> bSub (bin, x:xs) = (diff:diffs, bout)
>   where (diff, b)     = halfsub (x, bin)
>         (diffs, bout) = bSub (b, xs)
...
>   binary (sampleN mul) == (binary l1) * (binary l2)
>   where mul = multiplier (map lift0 l1, map lift0 l2)
...
> multiplier (s1,s2) = let (a,b) = (foldl (\(val,acc) s -> if (sample1 s) then ((adder (val,acc), low:acc)) else (val, low:acc)) ([],s2) s1) in a
